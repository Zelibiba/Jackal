...
namespace Jackal.Views
{
    public partial class GameView : ReactiveUserControl<GameViewModel>
    {
        ...
        GameViewModel _viewModel => DataContext as GameViewModel;
        double _cellSize => MapControl.Bounds.Height / Game.MapSize;

        Animation _pirateAnimation;
        TimeSpan _pirateAnimationDuration = TimeSpan.FromSeconds(0.3);
        ...
        private async void StartPirateAnimation(object? sender, CellArgs e)
        {
            ...
            PirateAnimator.Width = _cellSize / 3;
            ...
            for (int i = 0; i < 2; i++)
            {
                y[i] = cells[i].Row * _cellSize    + (pirateNumbers[i] / 3) * PirateAnimator.Bounds.Height + 7;
                x[i] = cells[i].Column * _cellSize + (pirateNumbers[i] % 3) * PirateAnimator.Bounds.Width + 3;
            }
